...
A good use-case for this hook is to track response time or log requests in combination with
`onGlobalRequest`.
...
export const requestLoggerPlugin = createApp()
  ...
  .onGlobalAfterResponse(({ method, route, set, startTime }) => {
    console.log(`${method} ${route} -> ${set.status} took ${Date.now() - startTime}ms`);
  })
  .export();
```
